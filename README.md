...
- **Let's Encrypt SSL** — Virtual host management with automatic certificate via acme.sh
...
| Docker | Latest | Official script |
| Composer | Latest | Official installer |
| WP-CLI | Latest | Official phar |
...
# PECL extensions to compile during install
...
lnmp vhost add            # Add virtual host (with optional SSL)
...
```bash
# Install during build (lnmp.conf)
...
sudo ./tools/addons.sh list            # Show available + status
...
sudo ./tools/backup.sh [dir]       # Backup DB + sites + configs
sudo ./tools/uninstall.sh          # Uninstall (preserves website files)
```
